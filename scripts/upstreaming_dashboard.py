...
def main():
    ...
    with open(f"{folder_path}/ready_to_upstream.md", 'w+') as writer:
        ...
        for file_path in project_files:
            ...
            text += f"* [`{module_name}`](https://github.com/YaelDillies/apap/blob/master/{file_path}) \n"
    ...
    with open(f"{folder_path}/easy_to_unlock.md", 'w+') as writer:
        ...
        for file_path in project_files:
            ...
            module_name = file_path.replace('/','.')[:-5]
            if num_sorries == 1:
                text += f"* [`{module_name}`](https://github.com/YaelDillies/apap/blob/master/{file_path}) {num_sorries} sorry\n"
            else:
                text += f"* [`{module_name}`](https://github.com/YaelDillies/apap/blob/master/{file_path}) {num_sorries} sorries\n"
